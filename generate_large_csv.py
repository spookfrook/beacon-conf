...
# Generate a larger CSV file to test the upload limit
def generate_large_csv(filename="test_5mb.csv", target_size_mb=5):
    headers = [
    ...
    with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        while current_size < target_size_bytes:
            row = [
                ...
                ''.join(random.choices(string.ascii_letters + string.digits + ' ', k=random.randint(50, 200))),
                f"Manager{random.randint(1, 50)}",
                f"Project-{random.randint(100, 999)}",
                random.randint(10000, 1000000),
                random.randint(0, 100),
                # Extra fields with more data
